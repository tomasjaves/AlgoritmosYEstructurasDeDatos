...
bool Polynomial::IsEqual(const Polynomial& pol, const double eps) const {
  ...
  if(this->get_size() != pol.get_size()) return differents;
  for(int i = 0; i < this->get_size(); ++i) {
    if(this->get_val(i) != pol.get_val(i)) return differents;
  }
  return !differents;
}
...
double SparsePolynomial::Eval(const double x) const {
  double result{0.0};
  for(int i = 0; i < this->get_nz(); ++i) {
    result += this->at(i).get_val() * pow(x,this->at(i).get_inx());
  }
  return result;
}
...
bool SparsePolynomial::IsEqual(const SparsePolynomial& spol
  ...
  bool differents = false;
  if(this->get_nz() != spol.get_nz()) return differents;
  for(int i = 0; i < get_nz(); ++i) {
    if(this->at(i).get_val() != spol.at(i).get_val()) return differents;    
  }
  return !differents;
}
...
bool SparsePolynomial::IsEqual(const Polynomial& pol, const double eps) const {
  bool differents = false;
  SparsePolynomial aux(pol);
  if(aux.get_nz() != this->get_nz()) return differents;
  for(int i = 0; i < this->get_nz(); ++i) {
    if(this->at(i).get_val() != aux.at(i).get_val()) return differents;
  }
  return !differents;
}
